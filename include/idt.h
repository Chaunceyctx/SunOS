...
typedef 
struct idt_entry_t {
	uint16_t base_low;	//中断处理函数地址
	uint16_t sel;		//目标代码段描述符选择子(到GDT中找到代码段基址)
	uint8_t  always0;	//置0段
	uint8_t  flags;		//DPL, door type, P
	uint16_t base_high;	//中断处理函数地址
...
//寄存器类型
typedef struct pr_regs_t {
	uint32_t ds;		//用于保存用户的数据段描述符
	uint32_t edi;		//将edi到eax寄存器内容通过push指令压入栈
	uint32_t esi;		
	uint32_t ebp;
	uint32_t esp;
	uint32_t ebx;
	...
	uint32_t eax;
	uint32_t int_no;	//中断号
	uint32_t err_code;	//若有中断错误代码，也压入栈
	uint32_t eip;		//下面由处理器自动压入
	uint32_t cs;
	uint32_t eflags;
	uint32_t useresp;
	uint32_t ss;
...
//申明中断处理函数0～19（属于CPU的异常终端）
//ISR：中断服务例程（interrupt service routine）
void isr0();			//0 #DE 除以0产生异常
void isr1();			//1 #DB 调试异常
void isr2(); 			// 2 NMI
void isr3(); 			// 3 BP 断点异常
void isr4(); 			// 4 #OF 溢出
