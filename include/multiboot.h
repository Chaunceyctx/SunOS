...
typedef struct multiboot_t{
	 ...
	 *从BIOS获知可用的内存
	 *
	 *mem_lower和mem_upper分别指出低端和高端的内存大小，单位是KB
	 *低端内存的首地址是0，高端内存的首地址是1M
	 *低端内存的最大可能值为640K	BIOS固件以上开始部分
	 *高端内存的最大可能值算是内存最大容量减去1M。但又不能保证是
	 */
	uint32_t mem_lower;
	uint32_t mem_upper;
	 
	uint32_t boot_device;	//指出引导程序从哪个磁盘设备载入映像
	...
	/*
	  *ELF格式内核的section头表，包括每项的大小,一共有几项以及作为名字索引的字符串(.text,.data.bss...)
  	 */
	uint32_t num;
	uint32_t size;
	uint32_t addr;
	...
	uint32_t drivers_length;	//指出第一个驱动器结构的物理地址
	uint32_t drives_addr;	//之处第一个驱动器这个结构的大小
...
typedef 
struct mmap_entry_t {
	uint32_t size;	//size 存着不含size自身变量的大小
