...
;中断服务程序
isr_common_stub:
	pusha			;把edi, esi, ebp, esp, ebx, edx, ecx, eax
	mov ax, ds
	push eax		;保存数据段描述符
	...
	push esp		;此时esp寄存器的值为pt_regs结构体指针
	call isr_handler	;调用C语言函数
	add esp, 4		;清除压入的参数
	
	pop ebx			;恢复原来的数据段描述符i
	;栈单位为32位，所以必须由32位保存，然后用其低16位
	mov ds, bx
	mov es, bx
	mov fs, bx
	mov gs, bx
	...
	popa			;弹栈：edi,esi,ebp, esp, ebx, edx, ecx, eax
	add esp, 8		;清理栈内的error code和isr
	iret
